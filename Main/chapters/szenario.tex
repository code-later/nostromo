...
\end{figure}
  ...
  In diesem Szenario sind von dem Anwendungsentwickler eine Reihe von Schritten notwendig, um die Funktionalität am Ende liefern zu können. Der erste Schritt ist dabei, dass er den Workflow der Anwendung definieren muss. Dies geschieht in Form einer Beschreibungsdatei (Schritt \emph{a)} in der Abbildung). Anschließend implementiert er die, an dem Workflow beteiligten Services. In diesem Falle also einen \emph{Producer} Service für die Erfassung der Videodaten und einen \emph{Consumer} Service für die Darstellung der Videodaten (Schritte \emph{b)} und \emph{d)} in der Abbildung). Die Vermittlung der Daten wird dabei von dem \emph{Medien Broker} übernommen. Abschließend müssen die beiden Dienste nur noch auf den entsprechenden Systemen an den jeweiligen Standorten installiert werden (Schritte \emph{c)} und \emph{d)} in der Abbildung).
  
  Das Ergebnis der Implementierung ist, dass nach Starten des Workflows der \emph{Producer} Service beginnt, Daten von der angeschlossenen Videokamera abzugreifen und diese Daten über den \emph{Medien Broker} bekannt zu geben. Der \emph{Consumer} Service kann anschließend wieder über den \emph{Medien Broker} auf diese Daten zugreifen und sie darstellen. Eine unidirektionale Videoverbindung zwischen zwei Endpunkten ist hergestellt.
