...
% subsubsection definitionen (end)
  ...
  Die Architektur einer Software ist also die strukturelle Aufteilung der Software in einzelne, von einander unabhängige Komponenten, deren Eigenschaften und Beziehungen untereinander. 
...
\paragraph{Definition: Dienst} % (fold)
\label{par:dienst}
...
% paragraph dienst (end)

\paragraph{Definition: Dienstorientierung} % (fold)
\label{par:definition_dienstorientierung}
...
% paragraph definition_dienstorientierung (end)

\paragraph{Definition: dienstorientierte Architektur} % (fold)
\label{par:definition_dienstorientierte_architektur}
...
- Eigenschaften einer SOA

% paragraph definition_dienstorientierte_architektur (end)

  - Einordnung in SOC nach Papazoglou
  - Einfache und erweiterte SOA
...
% subsubsection definitionen_soa (end)

% subsection definition_dienst_und_dienstorientierung (end)

  Nach dieser begrifflichen Einordnung der wesentliche Begriffe, werden im Folgenden nun die einzelnen Komponenten der COSIMA Architektur vorgestellt.
...
\label{sub:medienverarbeitende_komponenten}
  ...
  HIER AUCH AUF SAMMA'S ARBEIT VERWEISE~\citep[S. 23]{samma08}.
