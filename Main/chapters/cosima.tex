...
\subsubsection{Definition: Dienst} % (fold)
...
\subsubsection{Definition: Dienstorientierung} % (fold)
...
\subsubsection{Definition: Dienstorientierte Architektur} % (fold)
...
\label{sub:integration_von_medien}
  ...
  - Medien und Multimediasysteme beschreiben
  - Auf Besonderheiten eingehen
  - dann zu Synchronisation kommen
...
\label{ssub:synchronisation}
  ...
  - unterschiedliche Ebenen der Synchronisation.
  - Unterschiedliche Modelle zur Synchronisation, wie in~\citep[S. 28ff]{bericht} beschrieben, sollten Beachtung finden
  - Betrachtung von örtlichen, inhaltlichen sowie zeitlichen Beziehungen
  - Natürliche und synthetische Synchronisation~\citep{little1991ms, steinmetz1992mst}
  - intra- und interobjekt Synchronisation\footnote{Die \emph{interobjekt} oder \emph{interstream} Synchronisation steht im Gegensatz zu der \emph{intraobjekt} oder \emph{intrastream} Synchronisation und wird bei~\citep[S. 613]{bertino1998tsm} näher beschrieben.} Nach ~\citep[S. 264]{wu2001svo} ist interobjekt, interstream und intrastream zu unterscheiden. Bei~\citep[S. 613]{bertino1998tsm} werden die Begriffe interstream und intraobjekt bzw. intraobjekt und intraobjekt synonym verwendet. Hierzu unbedingt bei Steinmetz genauer nachsehen~\citep{steinmetz1995mcc}
  - intramedia (low-level) und intermedia (high-level) auch bei \citep{ackermann1996doo}
  
  \begin{figure}[!hb]
    \centering
      \includegraphics[width=.9\textwidth]{images/Erweiterte_Synchronisations-Ebenen.pdf}
    \caption{Erweiterte Ebenen der Synchronisation, nach~\citep[S. 13]{blakowski1996mss}}
  ...
  Synchronisation spielt im Kontext von Multimediaanwendungen also eine entscheidende Rolle und muss demnach explizit und prominent modelliert werden. Ein erster Ansatz zur Integration von Synchronisation in das COSIMA-Projekt sieht dazu eine dedizierte Komponente innerhalb der Servicekomposition vor. 
