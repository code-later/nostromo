...
\subsubsection{Integration von Medien} % (fold)
...
\paragraph{Medienobjekt} % (fold)
\label{par:medienobjekt}
  ...
  \begin{description}
    ...
    \item[Streaming] Beim Umgang mit Medien, vor allem mit kontinuierlichen Medien spielen fast immer auch Datenströme, oder auch \emph{Streaming} eine wesentliche Rolle~\citep[S. 14ff]{multimedia_technologie}. Aus diesem Grund muss das Medienobjekt Streaming explizit vorsehen. Eng verbunden mit dem Streaming ist auch die \emph{Dienstgüte}~\citep{multimedia_technologie} und muss daher ebenfalls berücksichtigt werden.
...
\end{figure}
  ...
  Um diese Ziele zu erreichen, wurde das Medienobjekt nach dem \emph{Composite} Design Pattern entworfen~\citep[S. 163]{design_patterns}. Abbildung~\ref{fig:medienobjekt} zeigt das stark vereinfachte Klassendiagramm des Medienobjektes. Viele Punkte sind immer noch offen, jedoch verspricht die jetzige Grundstruktur ein hohes Maß an Flexibilität bei der weiteren Umsetzung.

% paragraph medienobjekt (end)

\paragraph{Media Broker} % (fold)
\label{par:media_broker}

% paragraph media_broker (end)
...
\label{sub:offene_fragen}
  ...
  Nach~\citep[S. 8]{service_oriented_computing} stehen im Kontext des \emph{Service-Oriented Computing} (SOC) komponierte Dienste für weitere Kompositionen zur Verfügung. Diese Möglichkeit ist zur Zeit nicht explizit in COSIMA modelliert, wäre aber ein interessanter Punkt für die weitere Entwicklung. Hierbei müsste aber auch die Geschäftsprozesskomposition erneut betrachtet werden.
