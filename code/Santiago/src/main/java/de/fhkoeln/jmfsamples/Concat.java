 ...
 * contiguous output file. Whenever necessary, transocding may occur
 * automatically.
 */
public class Concat implements ControllerListener, DataSinkListener {
   ...
   * this method will concatenate the input media files to generate a
   * single concatentated output.
   */
  ...
  /**
   * Try to match all the tracks and find common formats to
   * concatenate the tracks. A database of results will be generated.
   */
  public boolean matchTracks(ProcInfo pInfo[], ContentDescriptor cd) {
    ...
    // of all the movies. We'll also need to disable the unused
    // tracks.
    ...
    for (i = 0; i < pInfo.length; i++) {
      for (type = AUDIO; type < MEDIA_TYPES; type++) {
        for (j = total[type]; j < pInfo[i].numTracksByType[type]; j++) {
          ...
          System.err
              .println("- Disable the following track since the other input media do not have a matching type.");
    ...
    for (type = AUDIO; type < MEDIA_TYPES; type++) {
      for (i = 0; i < total[type]; i++) {
        if (!tryMatch(pInfo, type, i)) {
          System.err
              .println("- Cannot transcode the tracks to a common format for concatenation!  Sorry.");
          return false;
        }
      }
    }
        ...
        for (int j = 0; j < pInfo.length; j++) {
          ...
          if (oldFmt instanceof VideoFormat) {
            ...
            if (oldSize != null && !oldSize.equals(newSize)) {
              ...
              System.err.println("  from: " + oldSize.width + " x "
                  + oldSize.height);
              System.err.println("  to: " + newSize.width + " x "
                  + newSize.height);
              newFmt = (new VideoFormat(null, newSize, Format.NOT_SPECIFIED,
