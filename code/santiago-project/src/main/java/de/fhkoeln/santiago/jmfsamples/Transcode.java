...
public class Transcode implements ControllerListener, DataSinkListener {
  ...
  /**
   * Given a source media locator, destination media locator and an
   * array of formats, this method will transcode the source to the
   * dest into the specified formats.
   */
  public boolean doIt(MediaLocator inML, MediaLocator outML, Format fmts[],
    ...
    try {
      ...
      System.err
          .println("Yikes!  Cannot create a processor from the given url: " + e);
      return false;
    }
  ...
  /**
   * Set the content descriptor based on the given output
   * MediaLocator.
   */
  ...
  boolean setTrackFormats(Processor p, Format fmts[]) {
    ...
    for (int i = 0; i < fmts.length; i++) {
      ...
      if (!setEachTrackFormat(p, tcs, fmts[i])) {
        System.out.println("- Trying format: " + fmts[i].toString());
  ...
  /**
   * Parse the audio format specifier and generate an AudioFormat. A
   * valid audio format specifier is of the form:
  ...
  /**
   * Parse the video format specifier and generate an VideoFormat. A
   * valid video format specifier is of the form:
