...
public class RotationEffect implements Effect {
  ...
  public Format setOutputFormat(Format output) {
    ...
    outputFormat = outputFormats[0]
        .intersects(new RGBFormat(size, maxDataLength, null, frameRate,
                                  Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
                                  Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
                                  Format.NOT_SPECIFIED, lineStride,
                                  Format.NOT_SPECIFIED, Format.NOT_SPECIFIED));
//    .intersects(VideoFormat(incoming.getEncoding(), incoming.getSize(),
//        incoming.getMaxDataLength(), incoming.getDataType(), frameRate));
  ...
  byte[] validateByteArraySize(Buffer buffer, int newSize) {
    ...
    if (objectArray instanceof byte[]) { // is correct type AND not
                                          // null
