...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <!-- ======================================================================================== -->
    <!-- Create content for lifelines -->
    <!-- ======================================================================================== -->
    <xsl:template name="createLifelineContent" match="lifeline" mode="createElementContent">
        ...
        <xsl:choose>
            <xsl:when test="@xsi:type='uml:Lifeline'">
            
                <xsl:apply-templates select="." mode="NamedElement">
                    <xsl:with-param name="titleKey" select="'Heading.Lifeline'"/>
            ...
            <xsl:otherwise>
                <!-- This is an error - since it means we have hit a lifeline that we do not know 
             how to generate contents for -->
                <xsl:message terminate="no">
                    <xsl:text>The current element was not handled by the
                        template createLifelineContent.</xsl:text>
