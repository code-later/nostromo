...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <!-- ======================================================================================== -->
    <xsl:template name="createMessageContent" match="message" mode="createElementContent">
        ...
        <xsl:choose>
            <xsl:when test="@xsi:type='uml:Message'">
            
                <xsl:apply-templates select="." mode="NamedElement">
                    <xsl:with-param name="titleKey" select="'Heading.Message'"/>
            ...
            <xsl:otherwise>
                <!-- This is an error - since it means we have hit a message that we do not know 
             how to generate contents for -->
                <xsl:message terminate="no">
                    <xsl:text>The current element was not handled by the
                        template createMessageContent.</xsl:text>
